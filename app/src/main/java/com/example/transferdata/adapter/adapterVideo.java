...
public class adapterVideo extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (this.folder) {
            ...
            ((RequestBuilder) ((RequestBuilder) Glide.with((Activity) this.context).load(((infoItemVideo) ((itemVideo) this.listVideo.get(position)).listVideo.get(0)).thumbnails).centerCrop()).thumbnail(0.1f).placeholder((int) R.color.cardview_light_background)).into(img);
        } else {
            clickCheckBoxVideo(checkBox, position);
            checkBox.setChecked(this.listVideo.get(0).listVideo.get(position).select);
            ((RequestBuilder) ((RequestBuilder) Glide.with((Activity) this.context).load(((infoItemVideo) ((itemVideo) this.listVideo.get(0)).listVideo.get(position)).thumbnails).centerCrop()).thumbnail(0.1f).placeholder((int) R.color.cardview_light_background)).into(img);
        }
        return row;
    }
    ...
    private void clickCheckBoxVideo(final CheckBox checkBox, final int pos) {
        checkBox.setOnClickListener(v -> {
            ...
            adapterVideo.this.context.adapterVideo.notifyDataSetChanged();
        });
    }
