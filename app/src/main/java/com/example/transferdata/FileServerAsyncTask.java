...
public class FileServerAsyncTask extends AsyncTask<Void, CustomObject, Void> {
    ...
    private FilesReceiveAdapter fileList;
    ...
    public FileServerAsyncTask(Context contextWeakReference,
                               ...
                               FilesReceiveAdapter fileListAdapterWeakReference, Callback callback) {
    ...
    protected void onProgressUpdate(CustomObject... values) {
        ...
        for (int i = 0; i < this.fileList.receivedFiles.length; i++) {
            if (this.fileList.receivedFiles[i].getName().equals(values[0].name)) {
                isNotContain = false;
                if (this.fileList.filesReceiveViewHolders.size() != this.fileList.receivedFiles.length) {
                    break;
                }
                FilesReceiveViewHolder holder = this.fileList.filesReceiveViewHolders.get(i);
...
//      serverSocket.close();
        ...
        try {
            if (this.fileList.filesReceiveViewHolders.size() != 0) {
                this.fileList.filesReceiveViewHolders
                        .get(this.fileList.filesReceiveViewHolders.size() - 1).progressBar.setVisibility(View.INVISIBLE);
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
